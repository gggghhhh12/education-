	...
	<div id="mainbody" >
		<div class="select">
			<div class="condition">
				<dl>
					<dt>求职地点</dt>
					<dd><input type="text" id="address" style="height: 30px;width: 130px" placeholder="求职地点"></input><a class="acity"></a>&nbsp;&nbsp;</dd>
				</dl>
				<dl>
					<dt>&nbsp;&nbsp;求职行业</dt>
					<dd><input type="text" id="trade" style="height: 30px;width: 130px" placeholder="求职行业"></input><a class="atrade"></a></dd>
				</dl>
				<dl>
					<dt>期望月薪</dt>
					<dd>
				...
				<div class="btn"><input type="button" value="搜索"></input></div>
			</div>
			<div class="selectResult">
				<div class="resultTitle">
					<span class="t1">姓名</span>
					<span class="t2">性别</span>
					<span class="t3">学校</span>
					<span class="t4">专业</span>
				</div>
		...
		<div class="recommend">
			...
			<div class="recdContent">
				...
				<c:if test="${!empty recommends }">
					<div class="single">
						<span class="s1">
							<a href="servlet/StuResumeDetailServlet?stuUsername=${recommends[0].stuUsername }" target="_blank">${recommends[0].stuName }</a>
						</span>
						<span class="s2">${recommends[0].sex }</span>
						<span class="s3">${recommends[0].school }</span>
						<span class="s4">${recommends[0].major }</span>
					</div>
					<div class="single">
						<span class="s1">
							<a href="servlet/StuResumeDetailServlet?stuUsername=${recommends[1].stuUsername }" target="_blank">${recommends[1].stuName }</a>
						</span>
						<span class="s2">${recommends[1].sex }</span>
						<span class="s3">${recommends[1].school }</span>
						<span class="s4">${recommends[1].major }</span>
					</div>
					<div class="single">
						<span class="s1">
							<a href="servlet/StuResumeDetailServlet?stuUsername=${recommends[2].stuUsername }" target="_blank">${recommends[2].stuName }</a>
						</span>
						<span class="s2">${recommends[2].sex }</span>
						<span class="s3">${recommends[2].school }</span>
						<span class="s4">${recommends[2].major }</span>
					</div>
					<div class="single">
						<span class="s1">
							<a href="servlet/StuResumeDetailServlet?stuUsername=${recommends[3].stuUsername }" target="_blank">${recommends[3].stuName }</a>
						</span>
						<span class="s2">${recommends[3].sex }</span>
						<span class="s3">${recommends[3].school }</span>
						<span class="s4">${recommends[3].major }</span>
					</div>
					<div class="single">
						<span class="s1">
							<a href="servlet/StuResumeDetailServlet?stuUsername=${recommends[4].stuUsername }" target="_blank">${recommends[4].stuName }</a>
						</span>
						<span class="s2">${recommends[4].sex }</span>
						<span class="s3">${recommends[4].school }</span>
			...
			<div class="tradeAndCount">
				...
				<c:forEach var="count" items="${counts }">
					...
					<span class="r3">
						占总人数比重:<fmt:formatNumber type="number" value="${count.count/sum*100 }" maxFractionDigits="2"></fmt:formatNumber>%
					</span>
