...
<html>
  <head>
	...
	<style type="text/css">
		...
		<%@ include file="./css/stuShowStore.css"%>
	</style>
    	...
    	<div class="store">
			...
			<div class="storeContent">
				<c:forEach var="epUsername" items="${epUsernames }">
					<div class="single">
						<span class="t1"><a href="servlet/StuJobDetail?EPusername=${epUsername.EPusername }&jobname=${epUsername.jobname }" target="_blank">${epUsername.jobname }</a></span>
						...
						<%} %>
						<span class="t3">${epUsername.jobaddr }</span>
						<span class="t4">${epUsername.salary }/月</span>
						<span class="t5" id="${epUsername.EPusername }___${epUsername.jobname }">取消收藏</span>
		...
		<<%-- div class="recommend">
			...
			<c:if test="${!empty recommends1 }">
				<div class="recdContent">
					<span class="s1">
						<a href="servlet/StuJobDetail?EPusername=${recommends1[0].EPusername }&jobname=${recommends1[0].jobname }" target="_blank">${recommends1[0].jobname }</a>
					</span>
					<span class="s2">${recommends1[0].salary }</span>
					<span class="s3">${recommends1[0].EPname }</span>
					<span class="s4">${recommends1[0].jobaddr }</span>
					<hr>
					<span class="s1">
						<a href="servlet/StuJobDetail?EPusername=${recommends1[1].EPusername }&jobname=${recommends1[1].jobname }" target="_blank">${recommends1[1].jobname }</a>
					</span>
					<span class="s2">${recommends1[1].salary }</span>
					<span class="s3">${recommends1[1].EPname }</span>
					<span class="s4">${recommends1[1].jobaddr }</span>
					<hr>
					<span class="s1">
						<a href="servlet/StuJobDetail?EPusername=${recommends1[2].EPusername }&jobname=${recommends1[2].jobname }" target="_blank">${recommends1[2].jobname }</a>
					</span>
					<span class="s2">${recommends1[2].salary }</span>
