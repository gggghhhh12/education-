...
<html>
				...
				$.get("servlet/stuSelectServlet2",{"salary":salary,"nature":nature,"diploma":diploma,"scale":scale},
					function(data,statusText){
						...
						for(var i=0;i<result.length;i++){
							...
							strResult += "<span class=\"t1\"><a href=\"servlet/StuJobDetail?EPusername=" + 
													result[i].epUsername + "&jobname="+result[i].jobName+"\" target=\"_blank\">" + result[i].jobName + "</a></span>" +
													"<span class=\"t2\">" + epname + "</span>" +
													"<span class=\"t3\">" + result[i].epAddr + "</span>" +
													"<span class=\"t4\">" + result[i].salary + "</span>" + "<hr>";
						}
  ...
  <%String userlevel = (String)session.getAttribute("userlevel"); %>
    ...
    <div id = "left">
  		...
  		<br>
  		<br>
  		<img src="images/robot.jpg" style="width:100%;height:auto;">
    ...
    <div id="mainbody">
    	<div class="select">
    		<form action="servlet/stuSelectServlet" method="post">
				<div class="text">
					<input id="address" type="text" name="address" style="height: 30px;width: 160px" placeholder="工作地点"></input>
					<a class="aplace"></a>
				</div>
				<div class="text">
					<input id="jobkind" type="text" name="jobkind" style="height: 30px;width: 160px" placeholder="职能类别"></input>
					<a class="ajobkind"></a>
				</div>
				<div class="text">
					<input id="trade" type="text" name="trade" style="height: 30px;width: 160px" placeholder="行业类别"></input>
					<a class="atrade"></a>
				</div>
				<div class="submit">
			...
			<div class="result">
				...
				<div class="resultcontext">
					<c:forEach var="ssr" items="${arrSsr }">
						...
						<span class="t1"><a href="servlet/StuJobDetail?EPusername=${ssr.EPusername }&jobname=${ssr.jobname }" target="_blank">${ssr.jobname }</a></span>
					<% 
						if(userlevel.equals("0"))
						{
					%>
						<span class = "t2">XXX公司</span>
					<%}else{ %>
						<span class="t2">${ssr.EPname }</span>
					<%} %>
						<span class="t3">${ssr.jobaddr }</span>
						<span class="t4">${ssr.salary }</span>
						<hr>
					</c:forEach>
				</div>
		...
		<div class="recommend">
			...
			<div class="newjob">
				<c:forEach var="lastestJob" items="${lastestJobs }">
					<dl>
						<dt><a href="servlet/StuJobDetail?EPusername=${lastestJob.epUsername }&jobname=${lastestJob.jobName  }" target="_blank">${lastestJob.jobName }</a></dt>
						<% 
						if(userlevel.equals("0"))
						{
						%>
			...
			<div class="tradeAndCount">
				...
				<c:forEach var="count" items="${counts }">
					...
					<span class="s3">
						占总量比重:<fmt:formatNumber type="number" value="${count.count/sum*100 }" maxFractionDigits="2"></fmt:formatNumber>%
					</span>
					<hr align="center" size="2px" color="#DDDDDD" noshade>
				</c:forEach>
			</div>
	...
	<div class="jobkind">
		...
		<dl>
			...
			<dd>
				...
				<span>系统分析师</span><br><span>数据库工程师</span><span>测试工程师</span><span>硬件工程师</span><span>网站营运</span><span>产品总监</span><span>产品主管</span><span>产品专员</span><span>项目总监</span><span>项目主管</span><span>项目专员</span><br><span>通信技术工程师</span><span>电信网络工程师</span>
				<span>其他</span>
			</dd>
		</dl>
		<dl>
