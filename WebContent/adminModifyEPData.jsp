    ...
    <div id="mainbody">
		...
		<div class="info">
			...
			<div class="basicinfo">
				<h4>公司资料</h4>
				<hr>
				<form action="servlet/AdminModifyEPData1Servlet" method="post">
					<input type = "hidden" name = "EPusername" value = "${EPData.EPusername }">
					<dl>
						<dt>公司名称</dt>
						<dd><input type="text" name="EPname" style="width:200px;height:30px" value="${EPData.EPname }"></dd>
					</dl>
					<dl>
						<dt>公司性质</dt>
						<dd>
							<select name="EPnature">
								<option value="股份制公司" <c:if test="${EPData.EPnature eq '股份制公司'}"> selected="selected"</c:if>>股份制公司</option>
								<option value="股份制合作制公司" <c:if test="${EPData.EPnature eq '股份制合作制公司'}"> selected="selected"</c:if>>股份制合作制公司</option>
								<option value="外资" <c:if test="${EPData.EPnature eq '外资'}"> selected="selected"</c:if>>外资</option>
								<option value="中外合资" <c:if test="${EPData.EPnature eq '中外合资'}"> selected="selected"</c:if>>中外合资</option>
								<option value="国营" <c:if test="${EPData.EPnature eq '国营'}"> selected="selected"</c:if>>国营</option>
								<option value="私营" <c:if test="${EPData.EPnature eq '私营'}"> selected="selected"</c:if>>私营</option>
							</select>
						</dd>
					</dl>
					<dl>
						<dt>公司机构代码</dt>
						<dd><input type="text" name="EPcode" style="width:180px;height:30px" value="${EPData.EPcode }"></dd>
					</dl>
					<dl>
						<dt>所属行业</dt>
						<dd><input id="EPtrade" type="text" name="EPtrade" style="width:180px;height:30px" value="${EPData.EPtrade }"><a class="atrade"></a></dd>
					</dl>
					<dl>
						<dt>公司规模</dt>
						<dd>
							<select name="EPscale">
								<option value="大型" <c:if test="${EPData.EPscale eq '大型'}"> selected="selected"</c:if>>大型</option>
								<option value="中型" <c:if test="${EPData.EPscale eq '中型'}"> selected="selected"</c:if>>中型</option>
								<option value="小型" <c:if test="${EPData.EPscale eq '小型'}"> selected="selected"</c:if>>小型</option>
							</select>
						</dd>
					</dl>
					<dl>
						<dt>公司所在地</dt>
						<dd><input id="EPaddr" type="text" name="EPaddr" style="width:180px;height:30px" value="${EPData.EPaddr }"><a class="acity"></a></dd>
					</dl>
					<dl>
						<dt>联系人</dt>
						<dd><input type="text" name="EPcontact" style="width:180px;height:30px" value="${EPData.EPcontact }"></dd>
					</dl>
					<dl>
						<dt>电子邮箱</dt>
						<dd><input type="text" name="EPemail" style="width:180px;height:30px" value="${EPData.EPemail }"></dd>
					</dl>
					<dl>
						<dt>固定电话</dt>
						<dd><input type="text" name="EPtel" style="width:180px;height:30px" value="${EPData.EPtel }"></dd>
					</dl>
					<dl>
						<dt>手机号码</dt>
						<dd><input type="text" name="EPmobile" style="width:180px;height:30px" value="${EPData.EPmobile }"></dd>
					</dl>
					<dl>
						<dt>邮政编码</dt>
						<dd><input type="text" name="EPpostalcode" style="width:180px;height:30px" value="${EPData.EPpostalcode }"></dd>
					</dl>
					<dl>
						<dt>公司简介</dt>
						<dd><textarea name="EPintroduction" style="width:300px;height:100px">${EPData.EPintroduction }</textarea></dd>
					</dl>
					<div class="button">
