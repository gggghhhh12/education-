		...
		$(document).ready(function(){
			...
			$(".tablelist a").click(function(){
				if($(this).text()=="通过审核"){
					...
					var jobname = $(this).attr("name");
					$.post("servlet/AdminChangeJobCheckServlet",{"EPusername":epUsername,"jobname":jobname},function(data,statusText){
					},"json");
				...
				if($(this).text()=="删除"){
					...
					var jobname = $(this).attr("name");
					var epUsername = $(this).attr("id");
					$.get("servlet/AdminDelEPPostJobServlet",{"epUsername":epUsername},function(data,statusText){
					},"json");
					$(this).parent().parent().empty();
				}
			});
  ...
  <input type="hidden" value="<%=times%>">
	...
	<div class="tablelist">
		<table style="width:98%; border-collapse:collapse; border-spacing:0;"> 
			<tr>
				<th width="10%">公司用户名</th><th width="15%">职位名</th><th width="10%">薪水</th><th width="15%">学历要求</th>
			...
			<%}else{ %>
				...
				<tr>
					...
					<td width="10%">${EPPostjob.jobaddr }</td>
					<td width="10%">
						<a id="${EPPostjob.EPusername }">删除</a>&nbsp;
						<a href = "servlet/AdminModifyEPPostJobServlet?epusername=${EPPostjob.EPusername }&jobname=${EPPostjob.jobname }">修改</a>&nbsp;
					</td>
				</tr>
			</c:forEach>
