  ...
  <body>
	...
	<div class="tablelist">
		<table style="width:98%; border-collapse:collapse; border-spacing:0;"> 
			<tr>
				<th width="20%">用户名</th><th width="20%">姓名</th>
				<th width="20%">性别</th><th width="20%">籍贯</th><th width="20%">操作</th>
			</tr>
			...
			<c:forEach var="basicInfo" items="${basicInfos }">
				<tr>
					<td width="20%">${basicInfo.username }</td><td width="20%">${basicInfo.name }</td>
					<td width="20%">${basicInfo.sex }</td><td width="20%">${basicInfo.residence }</td>
					<td width="20%">
						<a id="${basicInfo.username }">删除</a>
						<c:if test='${userlevels[basicInfo.username]=="0" }'>
							<input name="${basicInfo.username }" type="radio" value="0" checked="checked">普通
							<input name="${basicInfo.username }" type="radio" value="1">会员
						</c:if>
						<c:if test='${userlevels[basicInfo.username]=="1" }'>
							<input name="${basicInfo.username }" type="radio" value="0">普通
							<input name="${basicInfo.username}" type="radio" value="1" checked="checked">会员
						</c:if>
					</td>
				</tr>
			</c:forEach>
			...
			<c:forEach var="basicInfo" items="${basicInfos }">
				<tr>
					...
					<td width="20%">${basicInfo.sex }</td><td width="20%">${basicInfo.residence }</td>
					<td width="20%">
						<a id="${basicInfo.username }">审核通过</a>
					</td>
				</tr>
			</c:forEach>
			<%} %>
