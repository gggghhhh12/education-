  ...
  <body>
	...
	<div class="tablelist">
		<table style="width:98%; border-collapse:collapse; border-spacing:0;"> 
			<tr>
				<th width="10%">用户名</th><th width="15%">公司名</th><th width="10%">公司代码</th><th width="15%">性质</th>
				<th width="10%">公司地点</th><th width="15%">公司邮箱</th><th width="15%">联系电话</th><th width="10%">操作</th>
			</tr>
			<c:forEach var="EPdata" items="${EPDatas }">
				<tr>
					<td width="10%">${EPdata.EPusername }</td><td width="15%">${EPdata.EPname }</td><td width="10%">${EPdata.EPcode }</td><td width="15%">${EPdata.EPnature }</td>
					<td width="10%">${EPdata.EPaddr }</td><td width="15%">${EPdata.EPemail }</td><td width="15%">${EPdata.EPtel }</td>
					<td width="10%">
						<a id="${EPdata.EPusername }">删除</a>
						<c:if test='${EPlevels[EPdata.EPusername]=="0" }'>
							<input name="${EPdata.EPusername }" type="radio" value="0" checked="checked">普通
							<input name="${EPdata.EPusername }" type="radio" value="1">会员
						</c:if>
						<c:if test='${EPlevels[EPdata.EPusername]=="1" }'>
							<input name="${EPdata.EPusername }" type="radio" value="0">普通
							<input name="${EPdata.EPusername }" type="radio" value="1" checked="checked">会员
						</c:if>
					</td>
				</tr>
			</c:forEach>
